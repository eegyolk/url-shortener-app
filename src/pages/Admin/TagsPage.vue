        ...
        <div class="row full-width q-mb-lg">
          <div class="col-6 col-sm-6 row justify-start items-center">
            <div class="row items-center q-pr-sm q-pb-md">
              <q-icon size="md" name="sell" class="rotate-90" />
            </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getTitle = computed(() => {
      return `Tags (${tableRows.value.length})`;
    });

    const paginationOf = computed(() => {
      if (typeof qTableTags.value.pagination !== "undefined") {
        const { computedRows, computedRowsNumber } = qTableTags.value;
        const { page, rowsPerPage } = qTableTags.value.pagination;
    ...
    return {
      ...
      onUpdateHeaderCheckBox(value, ev) {
        ...
        qTableTags.value.computedRows.forEach(
