  ...
  <q-page>
    <div class="flex flex-center">
      ...
      <div v-if="tableRows.length" class="row full-width q-pa-lg">
        <q-table
          ref="qTableDomains"
          class="domains-table full-width"
...
export default defineComponent({
  ...
  setup() {
    const qTableDomains = ref(QTable);
    ...
    const getTitle = computed(() => {
      return `Domains (${tableRows.value.length})`;
    });

    const paginationOf = computed(() => {
      if (typeof qTableDomains.value.pagination !== "undefined") {
        const { computedRows, computedRowsNumber } = qTableDomains.value;
        const { page, rowsPerPage } = qTableDomains.value.pagination;
    ...
    return {
      ...
      onUpdateHeaderCheckBox(value, ev) {
        ...
        qTableDomains.value.computedRows.forEach(
